...
extern uint8_t flag_auto_iron, flag_auto_air;
...
void control_temperature_iron(void)
{
	if(HAL_GetTick() - timer_iron > 250) {
		...
		//
		if((float)target_iron > 0.0f && flag_auto_iron) {
			if((float)temperature_iron > (float)target_iron) {
...
void control_temperature_air(void)
{
	if(HAL_GetTick() - timer_air > 250) {
		...
		//
		if((float)target_air > 0.0f && flag_auto_air) {
